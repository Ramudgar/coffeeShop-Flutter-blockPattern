...
import 'package:aimsdigital/Models/Home/ProductHome.dart';
    ...
    return BlocListener<AdminProductBloc, AdminProductstate>(
      listener: (context, state) {
        if (state is LoadingAddProductstate) {
          modalLoading(context, 'Adding Coffee...');
        } else if (state is AddProductsuccessState) {
          Navigator.of(context).pop();
          modalFrave(context, 'Coffee Added Successfully');
        } else if (state is FailureAddProductstate) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error adding Coffee'),
              backgroundColor: Colors.red));
        } else if (state is LoadingUpdateProductstate) {
          modalLoading(context, 'Updating Coffee...');
        } else if (state is UpdateProductsuccessState) {
          Navigator.of(context).pop();
          modalFrave(context, 'Coffee Updated Successfully');
        } else if (state is FailureUpdateProductstate) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error Updating Coffee'),
              backgroundColor: Colors.red));
        }
      },
      child: Scaffold(
        ...
        appBar: AppBar(
            ...
            elevation: 0,
            title: CustomText(
                text: isUpdate ? "Update ${product.nameProduct}" : 'Add Coffee',
                color: Colors.black),
            centerTitle: true,
            leading: IconButton(
              icon: Icon(
        ...
        body: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              CustomText(text: 'Coffee Details', fontSize: 18),
              ...
              TextFormField(
                decoration: InputDecoration(hintText: "Enter Coffee Details"),
                initialValue: product.description,
                onChanged: (value) => product.description = value,
                validator: (value) {
                  if (value.isEmpty) {
                    return 'Please enter Coffee Details';
                  }
                  return null;
                },
              ),
