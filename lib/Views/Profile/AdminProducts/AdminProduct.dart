  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: CustomText(text: 'Manage Coffee', color: Colors.black),
        centerTitle: true,
        elevation: 0,
        leading: IconButton(
    ...
    return BlocListener<AdminProductBloc, AdminProductstate>(
      listener: (context, state) {
        if (state is DeleteProductLoadingState) {
          modalLoading(context, 'Deleting Coffee...');
        } else if (state is DeleteProductsuccessState) {
          setState(() {});
          Navigator.of(context).pop();
          modalFrave(context, 'Coffee Deleted Successfully');
        } else if (state is DeleteProductFailureState) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error Deleting Coffee'),
      ...
      child: Container(
        ...
        child: FutureBuilder<List<Product>>(
          ...
          builder: (context, snapshot) {
            print("This is snapsohot of Coffee data ${snapshot.data}");
